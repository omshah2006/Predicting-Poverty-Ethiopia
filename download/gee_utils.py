...
def make_tiles(img, scale, ksize, points, export, prefix, fname, selectors=None, dropselectors=None, bucket=None):
    ...
    return export_tiles(collection=samples, export=export, prefix=prefix,
...
def mask_l8_sr(img):
    ...
    return img.addBands(opt_bands, None, True).addBands(therm_bands, None, True).updateMask(qa_mask).updateMask(
        sat_mask)


def add_nl_band(img):
    nighttime_lights_col = ee.ImageCollection('NOAA/VIIRS/DNB/MONTHLY_V1/VCMCFG').filter(
        ee.Filter.date('2018-01-01', '2018-12-31')).median()
...
def create_ms_img(geometry):
    ...
    landsat_bands = ["SR_B1", "SR_B2", "SR_B3", "SR_B4", "SR_B5", "SR_B6", "SR_B7", "ST_B10", "QA_PIXEL"]
    renamed_bands = ["AEROSOL", "BLUE", "GREEN", "RED", "NIR", "SW_IR1", "SW_IR2", "TEMP", "QA_PIXEL"]
